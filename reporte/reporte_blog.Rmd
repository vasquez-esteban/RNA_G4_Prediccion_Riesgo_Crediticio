...
kable(tabla_logit,
  caption = "Tabla 1. Métricas de desempeño del modelo de referencia (regresión logística balanceada)",
  digits = 2,
  format = "html"
) %>%
  kable_styling(
...
kable(tabla_rna,
  caption = "Tabla 2. Métricas de desempeño del primer modelo de red neuronal",
  digits = 2,
  format = "html"
) %>%
  kable_styling(
...
kable(tabla_modelo2,
  caption = "Tabla 3. Métricas de desempeño del segundo modelo de red neuronal (100 épocas)",
  digits = 2,
  format = "html"
) %>%
  kable_styling(
...
kable(tabla_final,
  caption = "Tabla 4. Métricas de desempeño del modelo final entrenado con búsqueda de hiperparámetros",
  digits = 2,
  format = "html"
) %>%
  kable_styling(
...
```{r loss-auc-curves, out.width="100%", fig.align='center', fig.cap='Figura 10. Evolución de pérdida y AUC durante el entrenamiento del modelo final'}
...
kable(tabla_comparacion,
  caption = "Tabla 5. Comparación de métricas entre los modelos evaluados",
  digits = 3,
  format = "html"
) %>%
  kable_styling(
...
kable(tabla_scorecard,
  caption = "Tabla 4. Interpretación de los puntajes del scorecard crediticio",
  format = "html"
) %>%
...
![](risk_profiles_heatmap.png)  
*Figura X. Heatmap comparativo de variables clave por perfil de riesgo. Los tonos rojos indican mayor riesgo potencial.*
...
tabla_ejemplo_scorecard <- data.frame(
  `Caso` = c("Startup Tech", "Freelancer"),
  `Variables Clave` = c("`annual_inc`=$95k, `dti`=18%, `revol_util`=25%", 
                        "`annual_inc`=$32k, `dti`=43%, `inq_last_6mths`=5"),
  `Score` = c(790, 410),
  `Decisión` = c("Aprobado ($50k)", "Rechazado")
)

kable(tabla_ejemplo_scorecard,
  caption = "Tabla 5. Ejemplo real de aplicación de un scorecard crediticio",
  format = "html"
) %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE) %>%
  ...
  row_spec(0, bold = TRUE, color = "black", background = "lightgray")
```
*(Por poner un ejemplo, después se puede hacer uno con la página)*
...
- https://www.kaggle.com/code/estebanvasquezperez/rna-g4-predicci-n-riesgo-crediticio#RNA---B%C3%BAsqueda-de-hiperpar%C3%A1metros
